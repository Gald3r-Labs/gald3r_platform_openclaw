...
<h1 align="center">Gald3r for OpenClaw</h1>

<p align="center">
  The <strong>OpenClaw edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside OpenClaw</strong>. Part of the Gald3r framework
...
This is the **OpenClaw distribution** of Gald3r. The [`openclaw/`](./openclaw/) folder is a
complete, ready-to-deploy gald3r setup tuned for OpenClaw -- drop its contents into your
...
cp -r gald3r_platform_openclaw/openclaw/. /path/to/your/project/
```

Then open the project in OpenClaw and type `@g-status` to confirm gald3r loaded.

> The `openclaw/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
